...
WrapperSubject.prototype.containsMatchingElement = function (node) {
  if (!this.value.containsMatchingElement(node)) {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to contain matching node ' +
      format(node) + '.');
  }
  return this;
};
...
WrapperSubject.prototype.containsAllMatchingElements = function (nodes) {
  if (!this.value.containsAllMatchingElements(nodes)) {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to contain all matching elements ' +
      format(nodes) + '.');
  }
  return this;
};
...
WrapperSubject.prototype.containsAnyMatchingElements = function (nodes) {
  if (!this.value.containsAnyMatchingElements(nodes)) {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to contain any matching elements ' +
      format(nodes) + '.');
  }
  return this;
};
...
WrapperSubject.prototype.containsNoMatchingElements = function (nodes) {
  if (this.value.containsAnyMatchingElements(nodes)) {
    this.failureStrategy.fail('Expected ' + this.describe() +
      ' to contain no matching elements of ' + format(nodes) + '.');
  }
  return this;
};

/** Ensure that the subject element is empty.
    @return {WrapperSubject} `this` for chaining.
*/
WrapperSubject.prototype.isEmpty = function () {
  if (!this.value.isEmpty(selector)) {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to be empty.');
  }
  return this;
};

/** Ensure that the subject element is not empty.
    @return {WrapperSubject} `this` for chaining.
*/
WrapperSubject.prototype.isNotEmpty = function () {
  if (this.value.isEmpty(selector)) {
    this.failureStrategy.fail('Expected ' + this.describe() + ' to not be empty.');
  }
  return this;
};
